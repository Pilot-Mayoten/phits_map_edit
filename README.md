...
-   **マップの保存・読み込み:** 作成したマップをJSON形式で保存・復元可能。同じ条件で複数回の実験を効率的に実施できます。
-   **環境シミュレーションの自動実行:** 作成したマップに基づいて、PHITSによる空間全体の線量分布計算をGUIから直接実行。
-   **A\*アルゴリズムによる最適経路探索:** 環境線量マップを考慮し、移動距離と被ばく線量のバランスを取った最適経路を自動で探索。
-   **複数経路の管理:** スタート・ゴールは共通で、評価ステップ幅などが異なる複数の移動経路シナリオを定義・管理。経路ごとに被ばく回避の重み係数と合計線量を表示。
-   **経路ごとの結果保存:** 複数経路の計算結果を個別に保持し、後から任意の経路のプロファイルを表示可能。
-   **詳細評価用ファイルの一括生成:** 定義した経路に沿って、ロボット（検出器）を模擬した多数のPHITS入力ファイルを一括で生成。
-   **PHITSの一括実行と結果プロット:** 生成された詳細評価用ファイルを一括で実行し、経路ごとの積算線量を自動でグラフ化。
-   **経路の2D可視化:** 定義した経路と線源の位置関係を2Dグラフで視覚的に確認。壁（コンクリート製）も表示されます。
-   **結果データへのアクセス:** 線量プロファイルグラフの表示やCSV形式での結果ダウンロードが可能。
...
├── config_loader.py            # config.iniの読み込みと設定管理
├── config.ini                  # アプリケーション設定ファイル（ウィンドウサイズ、パス設定など）
...
├── utils.py                    # 座標変換などの共通関数、マップの保存・読み込み
├── results_exporter.py         # CSV形式での結果出力
...
1.  右側の**経路の管理**パネルにある「**経路を追加**」ボタンを押し、評価したい経路をリストに追加します。
    ...
    -   経路リストには、各経路の **被ばく回避の重み係数（係数ω）**、**ステップ幅**、**合計線量** が表示されます。
...
1.  **経路の管理**パネルでプロファイルを見たい経路を**1つ選択**します。
2.  「**線量プロファイルを表示**」ボタンを押すと、選択した経路の線量プロファイルグラフが表示されます。
    -   複数経路の計算後でも、各経路ごとに保存済み結果が表示されます。
3.  「**結果をExcelで開く**」ボタンでCSV形式の結果ファイルを選択して開くことができます。
...
アプリケーション全体の設定は `config.ini` ファイルで管理されています。以下の項目をカスタマイズできます。

```ini
[UI]
window_width = 1600      # ウィンドウの幅 (ピクセル)
window_height = 1080     # ウィンドウの高さ (ピクセル)
grid_width = 1050        # マップグリッドの表示幅 (ピクセル)
control_panel_width = 300 # 制御パネルの幅 (ピクセル)

[PHITS]
command = phits.bat      # PHITSの実行コマンド
default_maxcas = 10000   # デフォルトのシミュレーション粒子数
default_maxbch = 10      # デフォルトのバッチ数

[Visualization]
font_directory = C:/Windows/Fonts  # 日本語フォントのディレクトリ
font_files = meiryo.ttc,msgothic.ttc,yugothb.ttc  # 優先フォントリスト
```

ウィンドウサイズを変更したい場合や、使用するフォントをカスタマイズする場合は、このファイルを編集して保存し、アプリケーションを再起動してください。
